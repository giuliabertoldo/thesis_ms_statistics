library(metafor)
# Note: use test = "t" and dfs = "contain" (https://wviechtb.github.io/metafor/reference/misc-recs.html)


## PET = Egger regression (outcome is d_hat_jk and predictor is sqrt(sigma2_r))
pet <- function(df_meta, verbose){ # tol, iter,

  pet_results_names <- c("error", "pet_slope", "se_pet_slope", "p_pet_slope", "pet_int", "se_pet_int", "p_pet_int")
...
peese <- function(df_meta, verbose){ # tol, iter,
  ...
  peese_results_names <- c("error", "peese_slope", "se_peese_slope", "p_peese_slope", "peese_int", "se_peese_int", "p_peese_int")
  ...
  if (class(peese) =="try-error") {
    peese_results$error <- 1
...
pet_var_stab <- function(df_meta, verbose){ # tol, iter,
  ...
  pet_st_results_names <- c("error", "pet_st_slope", "se_pet_st_slope", "p_pet_st_slope", "pet_st_int", "se_pet_st_int", "p_pet_st_int")
  ...
  if (class(pet_st) =="try-error") {
    pet_st_results$error <- 1
...
peese_var_stab <- function(df_meta, verbose){ # tol, iter,
  ...
  peese_st_results_names <- c("error", "peese_st_slope", "se_peese_st_slope", "p_peese_st_slope", "peese_st_int", "se_peese_st_int", "p_peese_st_int")
  ...
  if (class(peese_st) =="try-error") {
    peese_st_results$error <- 1
