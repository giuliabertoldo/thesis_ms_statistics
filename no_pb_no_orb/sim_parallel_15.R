...
fun1 <- function(idx){
  ...
  conditions <- read_csv(file = 'conditions_0.2_15_0.1_0.1.csv')
  ...
  delta_00 <- local_condition[[1]]
  num_studies <- local_condition[[2]]
  sigma2_u <- local_condition[[3]]
  sigma2_v <- local_condition[[4]]
  psss <- local_condition[[5]]
  o <- local_condition[[6]]
  ...
  performance_analyzer(delta_00 = delta_00,sigma2_u = sigma2_u, sigma2_v = sigma2_v, psss = psss, o = o, k = num_studies, nmeta = 1000, verbose = FALSE) # tol = 1e-3, iter = 50,
