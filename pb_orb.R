...
pb_orb <- function(df, bias_type){
  ...
  for (i in seq(1:dim(df)[1])) {
    ...
    if ((bias_type == "pb_mod_orb_mod")|(bias_type == "pb_str_orb_mod")) {

      # Calculate probability of inclusion
      df$prob_incl[i] <- bias_moderate(df$pval_t[i])

    } else if ((bias_type == "pb_mod_orb_str")|(bias_type == "pb_str_orb_str")) {
...
#   }
  ...
  for (j in 1:k){
    ...
    # Group by study
    temp <- df_biased[df_biased['study_id']==j, ]
  ...
  for (i in seq(1:dim(df_grouped)[1])) {
    ...
    if ((bias_type == "pb_mod_orb_mod") | (bias_type == "pb_mod_orb_str")) {

      # Calculate probability of inclusion
      df_grouped$prob_incl[i] <- bias_moderate(df_grouped$pval_t[i])

    } else if ((bias_type == "pb_str_orb_mod") | (bias_type == "pb_str_orb_str") ) {
