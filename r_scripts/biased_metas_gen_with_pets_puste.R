...
biased_metas_gen_with_pets <- function(o, k, nmeta, delta_00, sigma2_v, sigma2_u, verbose, save_path, psss, bias_type){
  ...
  for (m in 1:nmeta) {
    ...
    if(file.exists(meta_path)){
      ...
      df_biased <- orb_puste(df = df_indicator_bias, bias_type = bias_type )

      # Save biased datasets
      ...
      tot_num_out_original <- k*o
      ## Real number of studies in the meta
      k_real <- length(unique(df_biased$study_id))
      ## Real total number of outcomes in the meta
      tot_num_out_real <- dim(df_biased)[1]
      ## If there are more than zero outcomes
      if(tot_num_out_real > 0) {
        ...
        # Average, min, max number of outcomes per study in the meta
        avg_n_out_per_study <- 0
        min_n_out_per_study <- 0
        max_n_out_per_study <- 0

        # Percentage selected outcomes
        perc_out_selected <- 0

        # Percentage selected studies
        perc_stud_selected <- 0
      }
